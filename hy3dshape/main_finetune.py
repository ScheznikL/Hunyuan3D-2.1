...
def get_args():
    ...
    parser.add_argument("-u", "--update_every", type=int, default=1) # how often we need to update accumulation
    parser.add_argument("-st", "--steps", type=int, default=50000000)
    parser.add_argument("-e", "--max_epochs", type=int, default=1)
    parser.add_argument("-lr", "--base_lr", type=float, default=4.5e-6) # base learning rate 
...
if __name__ == "__main__":
    ...
    training_cfg = config.training
    print(f"DEBUG: Available keys in training_cfg: {training_cfg.keys()}")
    ...
    # Build trainer
    # Якщо у тебе 1 GPU і модель працює стабільно — залиш None.
    # Якщо ж отримуєш помилку Out of Memory (OOM), 
    # ти можеш змінити цей код і ввімкнути DeepSpeedStrategy(stage=2, offload_optimizer=True), 
    # щоб задіяти свою велику оперативну пам'ять (RAM).
    ...
    num_epoch = 1
    #steps = (train_data_count / batch_size) * num_epoch * GPUs
...
# To this:
# Model already loaded in __init__, don't pass ckpt to trainer with DeepSpeed
    # if args.num_gpus > 1 and args.deepspeed and training_cfg.ckpt_path:
    #     print(f"Weights loaded in model.__init__ from {training_cfg.ckpt_path}")
    #     trainer.fit(model, datamodule=data, ckpt_path=None)
    # else:
    #     trainer.fit(model, datamodule=data) #ckpt_path=training_cfg.ckpt_path, weights_only=False
        
    # Train
    if training_cfg.ckpt_path == '': 
        training_cfg.ckpt_path = None
    
    print(f"[INFO] Checkpoint is {training_cfg.ckpt_path}")
    trainer.fit(model, datamodule=data, ckpt_path=training_cfg.ckpt_path, weights_only=False)#
