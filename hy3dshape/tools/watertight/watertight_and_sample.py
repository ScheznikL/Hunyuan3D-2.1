...
def sharp_sample_pointcloud(mesh, num=16384):
    ...
    if edge_a.size == 0:
        # No sharp edges found
        print("Sharp vertices:", sharp_mask.sum())
        print("Sharp edges:", edge_a.size)
...
def Watertight(V, F, epsilon = 2.0/256, grid_res = 256):
    ...
    # Compute SDF at grid points using igl.signed_distance with pseudo normals TODO check
...
if __name__ == '__main__':
    ...
    export_obj = f'{name}_watertight.obj'

    if os.path.exists(export_surface) and os.path.exists(export_sdf):
        print(f"[SKIP] Outputs already exist for {name}")
        #return
        sys.exit(0)   # or `return` if inside a function


    print("---Prosessed args!!---")
    ...
    igl.writeOBJ(f'{name}_watertight.obj', mc_verts, mc_faces) # TODO check NO igl.write_obj
