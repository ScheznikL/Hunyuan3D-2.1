...
from .text_encoder import MT5TextEncoder
...
#     def __init__(self, clip_model, target_dim):
#         super().__init__()
#         self.clip = clip_model
#         self.proj = nn.Linear(clip_dim, target_dim)

#     def forward(self, text):
#         tokens = self.clip.encode_text(text)  # [B, L, clip_dim]
#         return self.proj(tokens)              # [B, L, hidden_dim]
...
def build_text_encoder(config):
    print(config)
    if config['type'] == 'MT5TextEncoder':
        return MT5TextEncoder(**config['kwargs'])
    else:
        raise ValueError(f'Unknown text encoder type: {config["type"]}')
...
class MultiModalConditioner(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.image_encoder = build_image_encoder(image_encoder)
        self.text_encoder = build_text_encoder(text_encoder)    
        self.drop_ratio = drop_ratio
        self.disable_drop = True

    def forward(self, image=None, mask=None, text=None, **kwargs):
        
        outputs = {
            'main': self.image_encoder(image, mask=mask, **kwargs),
    ...
    def unconditional_embedding(self, batch_size, **kwargs):
        outputs = {
            'main': self.image_encoder.unconditional_embedding(batch_size, **kwargs),
        }
        if self.text_encoder is not None:
            outputs["text"] = self.text_encoder.unconditional_embedding(batch_size)
        return outputs

  
class TextModalConditioner(nn.Module):
    def __init__(
        self,
        text_encoder=None,
        drop_ratio=0.0,
    ...
    def unconditional_embedding(self, batch_size, **kwargs):
        outputs = {
            'text': self.text_encoder.unconditional_embedding(batch_size)
        }
        return outputs    