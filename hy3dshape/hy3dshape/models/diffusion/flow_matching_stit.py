...
from ..part_structure_encoder import PartStructureEncoder
...
class Diffuser(pl.LightningModule):
    def __init__(
        ...
        *,
        ...
        # Replace with:
        self.ckpt_path = ckpt_path
        if ckpt_path is not None and ckpt_path.endswith('.ckpt'):
            # Single-GPU checkpoint - load manually
            self.init_from_ckpt(ckpt_path, ignore_keys=ignore_keys)
    ...
    def init_from_ckpt(self, path, ignore_keys=()):
        print("[DEBUG] In init_from_ckpt ")
        ckpt = torch.load(path, map_location="cpu")
        ### DEBUG: 
        print("[DEBUG] vvvv Analysing CKPT vvvv")
        for key in ckpt:
            print(key)
        print("[DEBUG] ^^^^ KEYS Done ^^^^")
    ...
    def on_load_checkpoint(self, checkpoint):
        ...
        """
        print("[DEBUG] In on_load_checkpoint ") 
    ...
    def on_train_batch_end(self, *args, **kwargs):
        if self.ema_config is not None:
            self.model_ema(self.model) # LitEma - weights decay normalize (&) - for cleaner geom
    ...
    def forward(self, batch):
        with torch.autocast(device_type="cuda", dtype=torch.bfloat16): #float32 for text
           # contexts = self.cond_stage_model(image=batch.get('image'), text=batch.get('text'), mask=batch.get('mask'))
            #cHecking prompt integration
            if 'prompt' not in batch:
                print(f"CRITICAL: Batch keys are {batch.keys()}")
                raise KeyError("The key 'prompt' is missing in the data batch!")
            
            #print(f"[INFO from diffuser] batch has prompt - {batch.get('prompt')}")         
        ...
        with torch.autocast(device_type="cuda", dtype=torch.float16):
            with torch.no_grad():
                latents = self.first_stage_model.encode(batch[self.first_stage_key], sample_posterior=True) # send surface , bool val
                latents = self.z_scale_factor * latents # we mul by const value!!
                # print(latents.shape)
...
class ProstheticDiffuser(Diffuser): 
    def __init__(
        self,
        *,
        # All original params passed through
        first_stage_config,
        cond_stage_config,
        denoiser_cfg,
        scheduler_cfg,
        ...
        pipeline_cfg=None,
        # NEW params only:
        n_parts: int = 4,           # top, bottom, cavity, connector
        cond_dim: int = 1536,
        cavity_noise_scale: float = 0.05,   # preserve fit
        shell_noise_scale: float = 0.20,    # allow restyling
        **kwargs
    ):
        super().__init__(
        ...
        # Fourier embed like OmniEncoder does for bbox
        self.pe = FourierEmbedder(num_freqs=num_freqs, include_pi=True)
        
        self.global_measurement_proj = nn.Sequential(
            nn.Linear(self.pe.get_dims(3), cond_dim),
            nn.RMSNorm(cond_dim),
            nn.GELU(),
        )
        # ── NEW external adapters only ──────────────────────────────
        # These train from scratch; Diffuser's __init__ already loaded
        # all pretrained weights for self.model, VAE, conditioner
        
        self.global_measurement_proj = nn.Linear(3, cond_dim)
        

    def forward(self, batch):
        """
        Extends parent forward() with:
        1. Part-aware measurement conditioning
        2. 3D prior noise initialization
        """
        with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
            # ── Standard conditioning (unchanged from parent) ──
            contexts = self.cond_stage_model(
            image=batch.get('image'),
            text=batch.get('prompt'),
            mask=batch.get('mask')
            ...
            if 'extents_norm' in batch:
                # Same Fourier embedding OmniEncoder uses for bbox
                pe_extents = self.pe(batch['extents_norm'].unsqueeze(1))  # [B,1,pe_dim]
                global_token = self.global_measurement_proj(pe_extents)   # [B,1,cond_dim]
                contexts['global_token'] = global_token
                
            
            # # Inject into contexts — this flows into Hunyuan3DDiT.forward()
            # # which does: cond = torch.cat([img_emb, text_emb], dim=1)
            # # We extend that cond stream:
            # contexts['global_token']  = global_token

        with torch.autocast(device_type="cuda", dtype=torch.float16):
            with torch.no_grad():
                # ── Standard VAE encode (unchanged) ──────────────
        ...
        with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
            # transport.training_losses calls self.model (the DiT)
            # which needs to handle the extended contexts
