...
class PointCrossAttentionEncoder(nn.Module):
    ...
    def sample_points_and_latents(self, pc: torch.FloatTensor, feats: Optional[torch.FloatTensor] = None):
        B, N, D = pc.shape
        num_pts = self.num_latents * self.downsample_ratio # default - 20
        print(f'[INFO PRINT] got {N} - number of points in the object BUT working with: {num_pts}')
        ...
        print(f'query_pc: {query_pc.shape}') # TODO comment
        ...
        print(f'[INFO PRINT] we pass query.view with shape: {query.view(B, -1, query.shape[-1]).shape}')
